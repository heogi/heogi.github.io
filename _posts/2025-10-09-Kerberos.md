...
2. AS-REP (KDC/AS > Client)
	- KDC의 DB에서 사용자의 Client Key를 사용하여 Pre-Auth Data를 복호화하여 Timestamp 유효성 검증
	- krbtgt 계정의 키로 암호화된 TGT(Ticket Granting Ticket) 전달
3. TGS-REQ (Client > KDC/TGS)
	- TGT를 이용하여 접근할 서비스에 대한 TGS 발급 요청
4. TGS-REP (KDC/TGS > Client)
	- KDC가 TGT를 복호화하여 유효한 사용자임을 확인 후 TGS 발급
5. AP-REQ
6. AP-REP
